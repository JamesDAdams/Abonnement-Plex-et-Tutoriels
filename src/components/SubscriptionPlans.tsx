  ...
  return <section className="py-16 bg-white dark:bg-gray-800 transition-colors duration-200" id="abonnements">
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-7xl mx-auto justify-items-center">
          {plans.map((plan, index) => <div key={index} className={`w-full max-w-md rounded-lg shadow-lg overflow-hidden h-full flex flex-col ${plan.highlighted ? 'border-2 border-indigo-500 transform scale-105 md:scale-105 z-10' : 'border border-gray-200 dark:border-gray-600'}`}>
              ...
              <div className="p-8 bg-white dark:bg-gray-700 flex-1 flex flex-col">
                ...
                <button className={`${'w-full py-4 rounded-lg mt-auto font-semibold text-lg'} ${plan.highlighted ? 'bg-indigo-500 text-white hover:bg-indigo-600' : 'bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-white hover:bg-gray-300 dark:hover:bg-gray-500'} transition-colors`}>
                  Souscrire
                </button>
              </div>
            </div>)}
